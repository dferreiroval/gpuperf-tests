    ...
    var DEFAULT_TESTS = [
        ...
        SUITE = {
            ...
            setEnvironment: function () {
                ...
                this.config = env.testConfig || window.PERF_TEST_CONFIG || DEFAULT_CONFIG;
                this.debug = env.debug || false;
                this.server = env.server || DEFAULT_SERVER;
                this.testPath = env.testPath || DEFAULT_TEST_PATH;
            },
            _getParams: function () {
            ...
            run: function (callback) {
                ...
                if (!this._ready) {
                    this._runWaiting = true;
                    return;
                } else {
                    this.runSuite();
                }
            },
            runSuite: function (callback) {
                var tests = this.testList,
                    config = this.config,
                    testName,
                    testConfig,
            ...
            _finishTests: function () {
                ...
                this._sendResults(results, this.callback);
            },
            _sendResults: function (results, callback) {
                var xhr = createCORSRequest('POST', this.server);
                xhr.setRequestHeader('Content-type', 'application/json');
                xhr.onload = function () {
                    callback(results, xhr.responseText);
                };
